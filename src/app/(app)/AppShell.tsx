...
function ShellLayout({ children }: { children: ReactNode }) {
  ...
  const toolbarButtonClasses =
    "inline-flex h-8 w-8 items-center justify-center rounded-lg text-foreground/80 transition-colors hover:text-foreground hover:bg-muted/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40";
    ...
    <>
      ...
      <SidebarInset className="bg-background h-dvh flex flex-col overflow-hidden">
        ...
        <div className="pointer-events-none fixed inset-x-0 top-3 z-40 flex items-start justify-between px-3">
          <div
            className={cn(
              "pointer-events-auto flex items-center gap-3 rounded-2xl border border-border/60 bg-background/90 p-1 text-foreground shadow-sm backdrop-blur",
        ...
        <div className="flex flex-1 flex-col min-h-0">
          <div id="chat-scroll-container" className="flex-1 overflow-y-auto pt-2">
            {children}
          </div>
        </div>
      </SidebarInset>
